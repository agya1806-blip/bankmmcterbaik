...
import {
  Bell, Wallet, TrendingUp, BarChart3, AlertTriangle,
  ArrowUpRight, ArrowDownRight, Save, DollarSign,
  Landmark, Smartphone, Pencil, Trash2, Home,
} from "lucide-react";
...
export default function PribadiKeluargaDashboard({ unitId, label, backHref }: Props) {
  ...
  const headingName = isPribadi ? (currentUser?.nama || "Saya") : "Keluarga";
  const headingColor = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const saldoCardGrad = isPribadi
    ? "bg-gradient-to-br from-[#008CEB]/10 to-[#00C9A7]/10 dark:from-[#008CEB]/5 dark:to-[#00C9A7]/5"
    : "bg-gradient-to-br from-rose-500/10 to-pink-500/10 dark:from-rose-500/5 dark:to-pink-500/5";
  const saldoIconBg = isPribadi ? "bg-[#008CEB]/20" : "bg-rose-100 dark:bg-rose-900/30";
  const saldoIcon = isPribadi
    ? <Wallet className="w-4 h-4 text-[#008CEB]" />
    : <Home className="w-4 h-4 text-rose-500" />;
  const saldoLabel = isPribadi ? "Total Saldo" : "Total Saldo Keluarga";
  const saldoTextColor = isPribadi ? "text-[#008CEB] dark:text-[#4DA3E0]" : "text-rose-500 dark:text-rose-400";
  const tabActiveColor = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const dpGradient = isPribadi ? "from-[#008CEB] to-[#00C9A7]" : "from-rose-500 to-pink-500";
  const dpTipeActive = isPribadi ? "bg-[#008CEB]" : "bg-rose-500";
  const dpBtnGrad = isPribadi ? "from-[#008CEB] to-[#00C9A7]" : "from-rose-500 to-pink-500";
  const dpSectionIcon = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const dpTitle = isPribadi ? "Dompet Saya" : "Dompet Keluarga";
  const dpWalletBg = isPribadi ? "bg-[#008CEB]/10" : "bg-rose-100 dark:bg-rose-900/30";
  const dpWalletColor = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const dpWalletAmt = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const dpHoverColor = isPribadi ? "hover:text-[#008CEB]" : "hover:text-rose-500";
  const laporanIconColor = isPribadi ? "text-[#008CEB]" : "text-rose-500";
  const laporanTitle = isPribadi ? "Laporan Keuangan" : "Laporan Keuangan Keluarga";
  const riwayatTxColor = isPribadi ? "text-[#008CEB]" : "text-rose-500";

  const transactions = useLiveQuery(() => db.transactions.where("bookOrBranchId").equals(unitId).toArray(), []) || [];
  const cashflows = useLiveQuery(() => db.cashflows.where("bookOrBranchId").equals(unitId).toArray(), []) || [];
  const wallets = useLiveQuery(() => db.wallets.where("bookOrBranchId").equals(unitId).toArray(), []) || [];
  const piutangList = useLiveQuery(() => db.piutang.where("bookOrBranchId").equals(unitId).toArray(), []) || [];
  ...
  const handleSaveWallet = async () => {
    ...
    if (editingWallet) {
      ...
      await db.wallets.add({
        id: crypto.randomUUID(), bookOrBranchId: unitId, unitId,
  ...
  return (
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-heading font-extrabold tracking-tight">
            <span className={headingColor}>{headingName}</span>
          </h1>
        </div>
      ...
      <div className={`premium-card p-4 ${saldoCardGrad}`}>
        <div className="flex items-center gap-2 mb-1">
          <div className={`w-7 h-7 rounded-lg ${saldoIconBg} flex items-center justify-center`}>
            {saldoIcon}
          </div>
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{saldoLabel}</span>
        </div>
        <p className={`text-xl font-heading font-extrabold ${saldoTextColor} tracking-tight`}>Rp{stats.totalSaldo.toLocaleString()}</p>
      </div>
      ...
      <div className="flex gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-x-auto">
        {(["ringkasan", "catat", "hutang", "laporan", "riwayat", "dompet"] as TabKey[]).map((tab) => (
          <button key={tab} onClick={() => tab === "catat" ? router.push(`/buku-${unitId}/cashflow`) : setActiveTab(tab)} className={`px-3 py-2 rounded-lg text-[10px] font-bold whitespace-nowrap transition-all flex-1 ${activeTab === tab ? `bg-white dark:bg-[#0F1926] ${tabActiveColor} shadow-sm` : "text-slate-400"}`}>
        ...
        <div className="flex flex-col gap-3">
          <div className="premium-card p-4">
            <div className="flex items-center gap-2 mb-3">
              <BarChart3 className={`w-4 h-4 ${laporanIconColor}`} />
              <span className="text-xs font-heading font-extrabold">{laporanTitle}</span>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {recentTx.map((tx) => (
                <div key={tx.id} className="premium-card p-3 flex items-center justify-between">
                  ...
                  <div className="text-right shrink-0 ml-2">
                    <p className={`text-xs font-heading font-extrabold ${riwayatTxColor}`}>Rp{tx.totalBruto.toLocaleString()}</p>
        ...
        <div className="flex flex-col gap-3">
          <div className="premium-card p-4">
            <div className="flex items-center gap-3 mb-3">
              <div className={`w-10 h-10 rounded-2xl bg-gradient-to-br ${dpGradient} flex items-center justify-center text-white shadow-md`}>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["KasTunai", "Bank", "EWallet"] as const).map((t) => (
                  <button key={t} onClick={() => setWalletTipe(t)}
                    className={`py-2 rounded-xl text-[10px] font-bold flex flex-col items-center gap-1 transition-all ${walletTipe === t ? `${dpTipeActive} text-white` : "bg-slate-100 dark:bg-zinc-800 text-slate-400"}`}>
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveWallet}
                  className={`flex-1 py-2.5 rounded-xl bg-gradient-to-r ${dpBtnGrad} text-white font-bold text-xs active:scale-[0.98] flex items-center justify-center gap-1.5`}>
          ...
          <div className="premium-card p-3 space-y-2">
            <div className="flex items-center gap-2 mb-1">
              <Wallet className={`w-4 h-4 ${dpSectionIcon}`} />
              <span className="text-xs font-bold">{dpTitle}</span>
                ...
                {wallets.map((w) => {
                  ...
                  return (
                    <div key={w.id} className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/50 flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-lg ${dpWalletBg} flex items-center justify-center ${dpWalletColor}`}>
                      ...
                      <div className="text-right shrink-0">
                        <p className={`text-[10px] font-extrabold ${dpWalletAmt}`}>Rp{w.saldo.toLocaleString()}</p>
                        <div className="flex gap-1 mt-0.5">
                          <button onClick={() => handleEditWallet(w)} className={`p-0.5 text-slate-400 ${dpHoverColor}`}>
