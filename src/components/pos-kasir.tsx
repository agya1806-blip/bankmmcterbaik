...
import { executeTransactionPipelineV4, type PipelineInputV4, ensureBranchWallets } from "@/engine/transaction-pipeline-v4";
...
const PAYMENT_CONFIG: Record<PaymentMethod, { label: string; desc: string; icon: React.ReactNode }> = {
  LUNAS: { label: "Lunas", desc: "Bayar penuh", icon: <Check className="size-3.5" /> },
  DP: { label: "Uang Muka", desc: "Bayar sebagian", icon: <CreditCard className="size-3.5" /> },
  PIUTANG: { label: "Piutang / Bon", desc: "Belum bayar", icon: <AlertTriangle className="size-3.5" /> },
};
...
function generateClientId(branch: BookOrBranch): string {
  const d = new Date();
  const ds = `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}${String(d.getDate()).padStart(2, "0")}`;
  return `POS-${branch.slice(-3).toUpperCase()}-${ds}-${String(Math.floor(Math.random() * 9000) + 1000)}`;
}
...
export default function PosKasir({ branchConfig, backHref }: PosKasirProps) {
  const { bookOrBranchId, title, subtitle, icon } = branchConfig;
  ...
  const liveInventory = useLiveQuery(
    () => db.inventory.where("bookOrBranchId").equals(bookOrBranchId).toArray(),
    [bookOrBranchId],
    [] as DbInventoryItem[]
  );
  const inventory = useMemo(() => liveInventory ?? [], [liveInventory]);

  const liveWallets = useLiveQuery(
    () => db.wallets.where("bookOrBranchId").equals(bookOrBranchId).toArray(),
    [bookOrBranchId],
    [] as { id: string; bookOrBranchId: BookOrBranch; namaDompet: string; saldo: number; tipe: string; isActive: boolean }[]
  );
  const dbWallets = useMemo(() => liveWallets ?? [], [liveWallets]);

  /* ─── Derived Data ─── */
  const branchWallets = useMemo(() => dbWallets.filter((w) => w.isActive), [dbWallets]);
  const mainWallet = useMemo(() => branchWallets.find((w) => w.tipe === "KasTunai") ?? branchWallets[0], [branchWallets]);
  ...
  /* ─── Auto-set dpAmount based on payment method ─── */
  useEffect(() => {
  ...
  /* ─── Init wallets on mount ─── */
  useEffect(() => {
    ensureBranchWallets(bookOrBranchId);
  }, [bookOrBranchId]);
  ...
  const bayar = useCallback(async () => {
    ...
    try {
      const trxId = generateClientId(bookOrBranchId);
  ...
  /* ═══════════════════════════════════════════
     SUCCESS SCREEN (framer-motion animated popup)
     ═══════════════════════════════════════════ */
        ...
        <div className="flex-1 flex items-center justify-center px-4 pb-32">
          ...
          >
            ...
            <motion.div
              ...
              className="size-20 rounded-full bg-gradient-to-br from-[#7B61FF] to-[#FF5C00] flex items-center justify-center mx-auto shadow-xl shadow-[#7B61FF]/30"
            >
              <motion.div
                initial={{ scale: 0, rotate: -90 }}
                animate={{ scale: 1, rotate: 0 }}
                transition={{ delay: 0.35, type: "spring", damping: 15, stiffness: 300 }}
              >
            ...
            <div className="relative">
              {[...Array(6)].map((_, i) => (
                ...
                >
                  <Sparkles className="size-3 text-[#FF5C00]" />
                </motion.div>
              ))}
            </div>
            ...
            <motion.p
              ...
              className="text-xl font-bold font-heading text-slate-900 dark:text-white tracking-tight"
            >
              Pembayaran Berhasil!
            </motion.p>
            ...
            <motion.p
              ...
              className="text-[11px] text-slate-400 dark:text-zinc-500 font-mono"
            >
              No. {successInvoice}
            </motion.p>
            ...
            <motion.p
              ...
              className="text-2xl font-bold font-heading text-[#FF5C00] tabular-nums"
            >
            ...
            {paymentMethod === "DP" && (
              ...
              >
                DP: {formatRupiah(dpValue)} · Sisa: {formatRupiah(remainingDebt)}
              </motion.p>
            )}
            ...
            {customerWA && (
              <motion.p
                ...
                className="text-[10px] text-[#7B61FF]/70 font-medium"
              >
                Poin loyalitas otomatis ditambahkan
              </motion.p>
            )}
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={resetCart}
              className="w-full h-12 rounded-2xl bg-gradient-to-r from-[#7B61FF] to-[#FF5C00] text-white text-sm font-bold shadow-lg shadow-[#7B61FF]/25 flex items-center justify-center gap-2 active:scale-[0.97] transition-transform"
            >
  ...
  return (
    <div className="max-w-md mx-auto w-full min-h-screen bg-[#F8F9FD] dark:bg-[#0B0C16] relative overflow-x-hidden pb-32 flex flex-col" style={WB}>
      {/* ── Header ── */}
      <div
        className="sticky top-0 z-30 px-4 pt-[calc(1rem+env(safe-area-inset-top))] pb-3"
      ...
      >
        <div className="flex items-center gap-3">
          <motion.button
            ...
            className="size-10 rounded-xl flex items-center justify-center border border-slate-200/60 dark:border-slate-800/60 active:scale-[0.97] transition-transform"
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white truncate font-heading">{title}</h1>
            <p className="text-[11px] font-normal italic text-slate-400 dark:text-zinc-500">{subtitle}</p>
          </div>
          <motion.button
            whileTap={{ scale: 0.9 }}
            className="relative size-10 rounded-xl flex items-center justify-center active:scale-[0.97] transition-transform"
      ...
      <div className="px-4 mt-2">
        <motion.div
          ...
          style={{
            x: balanceSwipeX,
            background:
              balanceView === 0 ? "linear-gradient(135deg, #7B61FF, #6366F1)"
            ...
            >
              {balanceView === 0 && (
                <>
                  ...
                  <p className="text-2xl font-bold text-white font-heading tabular-nums mt-1">
                    {formatRupiah(mainWallet?.saldo ?? 0)}
                  </p>
              ...
              {balanceView === 2 && (
                <>
                  ...
                  <p className="text-white/50 text-[10px] mt-1">
                    {branchWallets.map((w) => w.namaDompet).join(", ") || "Belum ada dompet"}
                  </p>
                </>
              )}
            </motion.div>
      ...
      <div className="flex-1 px-4 mt-4 space-y-4">
        ...
        <motion.button
          ...
          className="w-full h-11 rounded-2xl bg-gradient-to-r from-[#7B61FF] to-[#FF5C00] text-white text-xs font-extrabold shadow-lg shadow-[#7B61FF]/25 flex items-center justify-center gap-2 active:scale-[0.97] transition-transform"
          style={WB}
        >
        ...
        >
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-10 pl-9 pr-3 text-xs rounded-xl bg-slate-100 dark:bg-[#131527]/60 border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30"
            />
            {searchQuery && (
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[10px] font-medium transition-all active:scale-[0.97] ${
                !activeCategory
                  ? "bg-[#7B61FF]/20 text-[#7B61FF]"
                  : "bg-slate-100 dark:bg-[#131527]/60 text-slate-500 dark:text-slate-400 hover:bg-slate-200"
              }`}
            >
            ...
            <AnimatePresence>
              {filteredInventory.map((item) => (
                <motion.button
                  ...
                  className="rounded-2xl p-3 text-center space-y-1.5 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 active:scale-[0.97]"
                  style={{ background: GL, backdropFilter: "blur(16px)", ...WB, border: "1px solid rgba(255,255,255,0.6)" }}
                >
                  ...
                  {item.stok > 0 && (
                    <p className={`text-[9px] ${item.stok <= item.stokMin ? "text-rose-500 font-semibold" : "text-slate-400 dark:text-slate-500"}`}>
        ...
        >
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 h-10 px-3 text-xs rounded-xl bg-slate-100 dark:bg-[#131527]/60 border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30"
            />
            <input
              type="text"
              inputMode="tel"
              ...
              placeholder="No. WA"
              className="w-28 h-10 px-3 text-xs rounded-xl bg-slate-100 dark:bg-[#131527]/60 border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30"
            />
          </div>
        </div>
        ...
        {cart.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setCart([]); setInventoryLinks([]); }} className="text-[10px] text-rose-500/70 hover:text-rose-500 active:scale-[0.97] transition-transform">
                Hapus Semua
              </button>
            </div>
            <AnimatePresence>
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <button
                      ...
                      className="text-[11px] font-normal italic text-slate-400 dark:text-zinc-500 hover:text-[#7B61FF] transition-colors flex items-center gap-0.5 mt-0.5 active:scale-[0.97]"
                    >
                      <Pencil className="size-2.5" />
                      {e.spesifikasi ? (
                        <span className="italic truncate max-w-[150px]">{e.spesifikasi}</span>
                      ) : (
                        "Atur Spesifikasi"
                      )}
                    </button>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <motion.button
                      ...
                      className="size-7 rounded-lg bg-slate-100 dark:bg-[#131527] flex items-center justify-center hover:bg-slate-200 dark:hover:bg-[#131527]/80 transition-colors text-slate-500 dark:text-slate-400 active:scale-[0.97]"
                    >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <input
                      ...
                      className="w-20 h-8 px-1.5 text-xs font-bold text-right tabular-nums rounded-lg bg-slate-100 dark:bg-[#131527] border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30"
                    />
                    <motion.button
                      whileTap={{ scale: 0.9 }}
                      onClick={() => removeCartItem(i)}
                      className="size-7 rounded-lg bg-rose-500/10 flex items-center justify-center hover:bg-rose-500/20 transition-colors active:scale-[0.97]"
                    >
            ...
            {(Object.keys(PAYMENT_CONFIG) as PaymentMethod[]).map((key) => {
              ...
              return (
                <motion.button
                  ...
                  className={`flex-1 h-10 rounded-xl text-[10px] font-bold flex flex-col items-center justify-center gap-0.5 transition-all active:scale-[0.97] ${
                    paymentMethod === key
          ...
          {paymentMethod === "DP" && (
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="0"
                className="w-full h-10 px-3 text-xs rounded-xl bg-slate-100 dark:bg-[#131527]/60 border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30 tabular-nums"
              />
              {dpValue > 0 && (
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 space-y-1">
            ...
            <select
              ...
              className="w-full h-10 px-3 text-xs rounded-xl bg-white dark:bg-[#131527]/60 border border-slate-200/60 dark:border-slate-700/60 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#7B61FF]/30"
            >
              {branchWallets.map((w) => (
          ...
          <motion.button
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowQuickAdd(true)}
            className="h-10 px-3 rounded-xl bg-[#7B61FF]/20 text-[#7B61FF] hover:bg-[#7B61FF]/30 transition-colors text-xs font-medium flex items-center gap-1 shrink-0 active:scale-[0.97]"
          >
          ...
          <motion.button
            ...
            className="h-10 px-3 rounded-xl bg-amber-500/20 text-amber-600 dark:text-amber-400 hover:bg-amber-500/30 transition-colors text-xs font-medium flex items-center gap-1 shrink-0 active:scale-[0.97]"
          >
      ...
      {/* ── Floating Pay Button ── */}
      <div
        className="fixed bottom-0 left-0 right-0 z-40 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))]"
      ...
      >
        <div className="max-w-md mx-auto">
          <motion.button
            ...
            className="w-full h-14 rounded-2xl bg-gradient-to-r from-[#7B61FF] to-[#FF5C00] text-white text-sm font-extrabold shadow-xl shadow-[#7B61FF]/30 flex items-center justify-center gap-2 disabled:opacity-30 disabled:pointer-events-none active:scale-[0.97] transition-transform"
          >
            {isProcessing ? (
      ...
      <div
        className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 w-[90%] max-w-sm h-14 rounded-full px-2 flex items-center justify-between shadow-2xl border border-white/10 z-40"
        style={{ background: "rgba(0,0,0,0.95)", backdropFilter: "blur(16px)", ...WB }}
      >
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={() => router.push("/buku-usaha")}
          className="flex flex-col items-center gap-0.5 px-4 py-2 active:scale-[0.97] transition-transform"
        >
