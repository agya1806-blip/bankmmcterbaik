...
import {
  ...
  Heart, ArrowLeftRight, Truck, ClipboardList, Repeat, DollarSign,
} from "lucide-react";
    ...
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      <div className="flex items-center justify-between animate-fade-in">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/buku-bisnis")} className="w-10 h-10 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center scale-press">
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="premium-card p-4 border-amber-300/40 dark:border-amber-700/40">
              ...
              {stats.stokMenipisCount > 0 && (
                <button onClick={() => { router.push(`/buku-bisnis/${cabangSlug}/inventory`); setShowNotif(false); }} className="flex items-center gap-2 w-full text-left py-1.5 text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg px-2 -mx-2 transition-all">
              ...
              {stats.piutangDueSoon.length > 0 && (
                <button onClick={() => { router.push(`/buku-bisnis/${cabangSlug}/transaksi`); setShowNotif(false); }} className="flex items-center gap-2 w-full text-left py-1.5 text-[11px] font-medium hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg px-2 -mx-2 transition-all">
      ...
      {stats.stokMenipisCount > 0 && (
        <div className="premium-card p-4 border-amber-300/50 dark:border-amber-700/50 animate-slide-up" style={{ animationDelay: "450ms", animationFillMode: "backwards" }}>
          ...
          {stats.stokMenipisCount > 3 && (
            <button onClick={() => router.push(`/buku-bisnis/${cabangSlug}/inventory`)} className="mt-3 w-full py-2 rounded-xl bg-amber-50 dark:bg-amber-950/20 text-[10px] font-bold text-amber-600 dark:text-amber-400 flex items-center justify-center gap-1 hover:bg-amber-100 dark:hover:bg-amber-950/40 transition-all">
      ...
      <div className="grid grid-cols-4 gap-2.5">
        {[
          ...
          { label: "Periode", slug: "period", icon: <Calendar className="w-5 h-5 text-white" />, color: "from-sky-400 to-cyan-500", badge: 0 },
          ...
          { label: "Kurs Valuta", slug: "exchange-rate", icon: <DollarSign className="w-5 h-5 text-white" />, color: "from-emerald-400 to-teal-500", badge: 0 },
        ].map((btn, i) => (
          <button key={btn.slug} onClick={() => router.push(`/buku-bisnis/${cabangSlug}/${btn.slug}`)} className="premium-card premium-card-glow p-3 flex flex-col items-center gap-1.5 scale-press animate-slide-up relative" style={{ animationDelay: `${500 + i * 60}ms`, animationFillMode: "backwards" }}>
      ...
      <div className="premium-card premium-card-glow p-4 animate-slide-up" style={{ animationDelay: "650ms", animationFillMode: "backwards" }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => router.push(`/buku-bisnis/${cabangSlug}/transaksi`)} className="text-[10px] font-bold text-[#008CEB] flex items-center gap-1 hover:gap-1.5 transition-all">
            Lihat semua <ArrowRight className="w-3 h-3" />
          </button>
        </div>
        {recentTx.length === 0 ? (
