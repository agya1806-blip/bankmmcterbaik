...
export default function BukuKeluargaCashflowPage() {
  ...
  const cashflows =
    useLiveQuery(
      () => db.cashflows.where("bookOrBranchId").equals(BOOK_ID).reverse().toArray(),
      []
    ) || [];

  const wallets =
    useLiveQuery(
      () => db.wallets.where("bookOrBranchId").equals(BOOK_ID).filter(w => w.isActive).toArray(),
      []
    ) || [];
  ...
  return (
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <button
          onClick={() => router.push("/buku-keluarga")}
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-slate-800">
                <h3 className="text-sm font-extrabold">Catat Cashflow</h3>
                <button
              ...
              <form onSubmit={handleSave} className="space-y-3 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Contoh: Belanja bulanan, Uang jajan..."
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-2xl bg-gradient-to-r from-rose-400 to-pink-500 text-white font-extrabold text-xs shadow-lg active:scale-[0.98] transition-transform flex items-center justify-center gap-2"
                >
