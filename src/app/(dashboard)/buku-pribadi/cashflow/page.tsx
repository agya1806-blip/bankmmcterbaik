...
import { TrendingUp, TrendingDown, Wallet, Save, ArrowLeft, Plus, ChevronDown, ChevronUp, X, Pencil, Trash2 } from "lucide-react";
import { showToast } from "@/lib/toast";

const BOOK_ID: UnitId = "pribadi";

export default function BukuPribadiCashflowPage() {
  ...
  return (
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <button onClick={() => router.push("/buku-pribadi")} className="p-2 bg-white dark:bg-[#131527] rounded-full shadow-md active:scale-95 transition-transform">
          <ArrowLeft className="w-5 h-5 text-slate-500" />
        </button>
        <h1 className="text-lg font-heading font-extrabold tracking-tight">Cashflow Pribadi</h1>
          ...
          <form onSubmit={handleSave} className="space-y-3 text-xs">
            ...
            <div>
              ...
              <select value={kategori} onChange={(e) => setKategori(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-100 dark:bg-zinc-800 focus:outline-none font-bold">
                {["Umum", "Makanan", "Transport", "Belanja", "Tagihan", "Hiburan", "Gaji", "Lainnya"].map((k) => (
            ...
            <div>
              ...
              <input type="text" value={catatan} onChange={(e) => setCatatan(e.target.value)} placeholder="Contoh: Beli sembako, Gaji bulanan..." className="w-full px-3 py-2 rounded-xl bg-slate-100 dark:bg-zinc-800 focus:outline-none" required />
            </div>
