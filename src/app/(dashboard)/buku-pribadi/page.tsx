    ...
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      ...
      <div className="flex gap-1.5 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl overflow-x-auto">
        {([ "ringkasan", "catat", "hutang", "laporan", "riwayat", "dompet" ] as TabKey[]).map((tab) => (
          <button key={tab} onClick={() => tab === "catat" ? router.push("/buku-pribadi/cashflow") : setActiveTab(tab)} className={`px-3 py-2 rounded-lg text-[10px] font-bold whitespace-nowrap transition-all flex-1 ${activeTab === tab ? "bg-white dark:bg-[#0F1926] text-[#008CEB] shadow-sm" : "text-slate-400"}`}>
