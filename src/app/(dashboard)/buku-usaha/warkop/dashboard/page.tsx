...
import { Coffee, Plus, Settings, ArrowLeft } from "lucide-react";
import { CardSkeleton } from "@/components/ui/skeleton";

export default function DashboardWarkop() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20 space-y-5 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold font-heading">Dashboard Warkop</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">Warkop & Minuman</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => router.push("/buku-usaha/warkop/kasir")}
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400">Transaksi warkop</p>
        </button>
        <button
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400">Kelola menu & stok</p>
        </button>
      </div>
    </div>
  );
