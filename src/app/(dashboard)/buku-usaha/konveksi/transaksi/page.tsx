...
export default function KonveksiTransaksi() {
  const router = useRouter();
  return (
    <div className="space-y-4 pb-4">
      <div className="flex items-center gap-3">
        <button
          onClick={() => router.push("/buku-usaha/konveksi/dashboard")}
        ...
        <div>
          <h1 className="text-base font-bold font-heading">Aktivitas Konveksi</h1>
          <p className="text-xs text-muted-foreground/60">Riwayat transaksi & produksi</p>
        </div>
      </div>
      <BranchTransactionHistory branch={BRANCH} branchLabel="Konveksi" showProduction />
    </div>
  );
}
