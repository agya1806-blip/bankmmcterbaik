...
import { Printer, Smartphone, Monitor, Coffee, Shirt, Bell, Settings, Wallet, TrendingUp, TrendingDown, ShoppingCart, Clock, Calendar, FileText, BarChart3, Store, Eye, ChevronRight, X } from "lucide-react";
...
const BRANCHES: BranchInfo[] = [
  { slug: "percetakan", label: "Percetakan", color: "from-blue-500 to-blue-600", icon: <Printer className="w-5 h-5" />, bookId: "usaha-percetakan" },
  { slug: "gadget", label: "Gadget", color: "from-indigo-500 to-indigo-600", icon: <Smartphone className="w-5 h-5" />, bookId: "usaha-gadget" },
  { slug: "laptop", label: "Komputer & Laptop", color: "from-violet-500 to-purple-600", icon: <Monitor className="w-5 h-5" />, bookId: "usaha-laptop" },
  { slug: "warkop", label: "Kedai Kopi", color: "from-orange-400 to-orange-500", icon: <Coffee className="w-5 h-5" />, bookId: "usaha-warkop" },
  { slug: "konveksi", label: "Fashion & Konveksi", color: "from-pink-400 to-pink-500", icon: <Shirt className="w-5 h-5" />, bookId: "usaha-konveksi" },
];
  ...
  return (
    <div className="flex flex-col gap-4 pt-2 pb-4 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: "Hari Ini", value: `Rp${stats.totalHariIni.toLocaleString()}`, color: "emerald", icon: <Wallet className="w-5 h-5" /> },
          { label: "Total Transaksi", value: String(stats.jumlahTransaksi), color: "blue", icon: <ShoppingCart className="w-5 h-5" /> },
          { label: "Piutang Aktif", value: `Rp${stats.piutangAktif.toLocaleString()}`, color: "amber", icon: <Clock className="w-5 h-5" /> },
          { label: "Laba Bersih", value: `Rp${stats.labaBersih.toLocaleString()}`, color: stats.labaBersih >= 0 ? "emerald" : "rose", icon: stats.labaBersih >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" /> },
        ].map((s, i) => (
      ...
      <div className="premium-card premium-card-glow p-4 animate-slide-up" style={{ animationDelay: "350ms", animationFillMode: "backwards" }}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-violet-50 dark:bg-violet-950/20 p-3 rounded-xl">
            <span className="text-[9px] text-violet-600 font-bold uppercase flex items-center gap-1"><FileText className="w-4 h-4" /> Bulan Ini</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl">
            <span className="text-[9px] text-slate-500 font-bold uppercase flex items-center gap-1"><FileText className="w-4 h-4" /> Bulan Lalu</span>
      ...
      <div className="animate-slide-up" style={{ animationDelay: "450ms", animationFillMode: "backwards" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[9px] text-slate-400 font-bold ml-auto">{BRANCHES.length} cabang</span>
        </div>
